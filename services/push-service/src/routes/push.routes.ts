...
import { send_push_notification, validate_device_token } from "../utils/send_push.js";
import {
  ...
  fastify.post('/send-bulk', {
        ...
        200: {
          ...
          properties: {
            ...
            results: {
              ...
              description: 'Array of individual results for each notification'
            },
            summary: {
              type: 'object',
              properties: {
    ...
    try {
      ...
      const results = await Promise.all(
        notifications.map(notification => send_push_notification(notification))
      );
            ...
            capabilities: {
              ...
              properties: {
                maxBulkNotifications: {
                  type: 'number',
                  description: 'Maximum number of notifications in bulk request',
                  example: 100
                },
                supportedPlatforms: {
                  type: 'array',
                ...
                features: {
                  ...
                  example: ['rich_notifications', 'custom_data', 'priority', 'ttl']
                }
              }
            }
          }
